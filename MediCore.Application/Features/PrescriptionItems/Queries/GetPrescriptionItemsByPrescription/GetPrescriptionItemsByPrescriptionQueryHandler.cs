...
public class GetPrescriptionItemsByPrescriptionQueryHandler
    : IRequestHandler<
        GetPrescriptionItemsByPrescriptionQuery,
        IEnumerable<PrescriptionItemDto>>
{
    private readonly IPrescriptionItemRepository _repository;

    public GetPrescriptionItemsByPrescriptionQueryHandler(
    ...
    public async Task<IEnumerable<PrescriptionItemDto>> Handle(
        GetPrescriptionItemsByPrescriptionQuery request,
        CancellationToken cancellationToken)
    {
        var items = await _repository.GetByPrescriptionIdAsync(
