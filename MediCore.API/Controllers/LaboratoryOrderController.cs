...
public class LaboratoryOrderController : ControllerBase
{
    ...
    public async Task<ActionResult<ApiResponse<int>>> Create(
        CreateLaboratoryOrderCommand command)
    {
    ...
    public async Task<ActionResult<ApiResponse<IEnumerable<LaboratoryOrderDto>>>> GetAll()
    {
        var orders = await _mediator.Send(
            new GetAllLaboratoryOrdersQuery());

        return Ok(
            ApiResponse<IEnumerable<LaboratoryOrderDto>>
                .SuccessResponse(
                    orders,
    ...
    [HttpGet("{id:int}")]
    public async Task<ActionResult<ApiResponse<LaboratoryOrderDto>>> GetById(
        int id)
    {
        var order = await _mediator.Send(
            new GetLaboratoryOrderByIdQuery(id));

        return Ok(
            ApiResponse<LaboratoryOrderDto>
                .SuccessResponse(
                    order,
    ...
    public async Task<ActionResult<ApiResponse<bool>>> Update(
        UpdateLaboratoryOrderCommand command)
    {
        ...
        int id)
    {
        var result = await _mediator.Send(
            new DeleteLaboratoryOrderCommand(id));
